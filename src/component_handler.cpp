...
namespace Host {
    ...
    ComponentHandler::performEdit(Steinberg::Vst::ParamID id,
        ...
        std::println("Edit performed on param {} with value {}", id,
                     valueNormalized);
        return Steinberg::kResultTrue;
    }
    ...
    Steinberg::tresult
    ComponentHandler::restartComponent(Steinberg::int32 flags) {
        std::println("Component restarted");
        return Steinberg::kResultTrue;
    }

    Steinberg::tresult
    ComponentHandler::queryInterface(const Steinberg::TUID _iid, void **obj) {
