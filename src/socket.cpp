...
public:
    WriteSocket(const char *host, short port) : host{host}, port{port} {
        printf("constructor\n");
        ...
        /* int res = connect(sock_desc, (const sockaddr *)&server,
         * sizeof(server)); */
        /* if (res == -1) { */
    ...
    ~WriteSocket() {
        ...
        if (shutdown_res == -1) {
            std::printf("Failed to shutdown socket you suck. Destructors "
                        "aren't allowed to throw for some reason\n");
        }
    }
